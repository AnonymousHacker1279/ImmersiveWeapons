...
public class BlockModelGenerator extends ModelProvider {
	...
	protected void registerModels(BlockModelGenerators blockModels, ItemModelGenerators itemModels) {
		...
		blockModels.createTrivialBlock(BlockRegistry.BULLETPROOF_GLASS.get(), TexturedModels.TRIVIAL_CUBE_CUTOUT_MIPPED);
		blockModels.createTrivialBlock(BlockRegistry.CLOUD.get(), TexturedModels.TRIVIAL_CUBE_TRANSLUCENT);
		...
		blockModels.createTrivialBlock(BlockRegistry.STARDUST_LEAVES.get(), TexturedModels.STARDUST_LEAVES);

		// Generate data for simple, six-sized blocks that use overlays (ores)
		for (Block block : STONE_BASED_ORES) {
			blockModels.createTrivialBlock(block, TexturedModels.STONE_ORE);
		}
		for (Block block : DEEPSLATE_BASED_ORES) {
			blockModels.createTrivialBlock(block, TexturedModels.DEEPSLATE_ORE);
		}
		for (Block block : NETHERRACK_BASED_ORES) {
			blockModels.createTrivialBlock(block, TexturedModels.NETHERRACK_ORE);
		}
		// Astral ore uses smooth quartz for a base
		blockModels.createTrivialBlock(BlockRegistry.ASTRAL_ORE.get(), TexturedModels.SMOOTH_QUARTZ_ORE);
		// Void ore uses end stone for a base
		blockModels.createTrivialBlock(BlockRegistry.VOID_ORE.get(), TexturedModels.END_STONE_ORE);

		// Generate data for tables
		blocks.stream().filter(WoodenTableBlock.class::isInstance).forEach(table -> {
			blockModels.createTrivialBlock(table, TexturedModels.TABLE);
		});
		...
		/**
		 ...
		 BlockModelGenerators.TEXTURED_MODELS.put(BlockRegistry.BLOOD_SANDSTONE.get(), TexturedModel.TOP_BOTTOM_WITH_WALL.get(BlockRegistry.BLOOD_SANDSTONE.get()));
