...
public class ChampionKeycardBlock extends Block {
	...
	protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult result) {
		if (hand.equals(InteractionHand.MAIN_HAND)) {
			...
			if (player.getItemInHand(hand).getItem() == ItemRegistry.CHAMPION_KEYCARD.get()) {
				...
				propagateToNearbyBlocks(state, level, pos, player, hand, result);
	 ...
	 */
	private void propagateToNearbyBlocks(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
