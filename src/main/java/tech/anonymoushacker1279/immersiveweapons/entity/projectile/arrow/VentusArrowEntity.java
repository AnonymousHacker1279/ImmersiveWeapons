...
public class VentusArrowEntity extends CustomArrowEntity implements HitEffectUtils {

	public VentusArrowEntity(EntityType<? extends AbstractArrow> type, Level level) {
		super(type, level);
	}

	public VentusArrowEntity(LivingEntity shooter, Level level) {
		super(EntityRegistry.VENTUS_ARROW_ENTITY.get(), shooter, level);
	}

	public VentusArrowEntity(Level level, double x, double y, double z) {
		super(EntityRegistry.VENTUS_ARROW_ENTITY.get(), level, x, y, z);
	}
	...
	public @Nullable Item getReferenceItem() {
		return ItemRegistry.VENTUS_ARROW.get();
	}
