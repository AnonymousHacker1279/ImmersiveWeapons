...
public class BulletEntity extends AbstractArrow implements HitEffectUtils {
	...
	public static final EntityDataAccessor<Float> GRAVITY_MODIFIER_ACCESSOR = SynchedEntityData.defineId(BulletEntity.class,
		...
		if (inGround && !isNoPhysics) {
			...
			if (!isNoGravity() && !isNoPhysics) {
				...
				if (shouldStopMoving) {
					...
					double modifier = firingItem instanceof MusketItem ? gravityModifier / 4 : gravityModifier;
					setDeltaMovement(deltaMovement1.x, deltaMovement1.y - modifier, deltaMovement1.z);
				}
			}
