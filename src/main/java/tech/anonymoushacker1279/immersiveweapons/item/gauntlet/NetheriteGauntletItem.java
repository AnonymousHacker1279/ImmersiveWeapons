...
public class NetheriteGauntletItem extends GauntletItem {
	public static Multimap<Attribute, AttributeModifier> gauntletAttributes;
	private final float bleedChance;

	public NetheriteGauntletItem(Tier tier, int attackDamageModifier, float attackSpeedModifier, Properties properties, float bleedChance) {
	...
	public @NotNull Multimap<Attribute, AttributeModifier> getAttributeModifiers(EquipmentSlot equipmentSlot, ItemStack stack) {
		...
		if (PostSetupHandler.hasCompletedClientSetup) {
			returnValue = equipmentSlot == EquipmentSlot.MAINHAND ? NetheriteGauntletItem.gauntletAttributes : super.getAttributeModifiers(equipmentSlot, stack);
		} else {
			returnValue = NetheriteGauntletItem.gauntletAttributes;
		}
		return returnValue;
	}
	...
	public void bleedBehavior(LivingEntity target) {
		if (GeneralUtilities.getRandomNumber(0.0f, 1.0f) <= bleedChance) {
			target.addEffect(new MobEffectInstance(DeferredRegistryHandler.BLEEDING_EFFECT.get(), 200, 2, false, false));
		}
	}
}