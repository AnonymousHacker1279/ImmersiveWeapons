...
public class BarrelTapBlock extends HorizontalBlock {
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (blockstateNorth.isIn(Blocks.BARREL)) {
			directionToUse = "north";
		} else if (blockstateSouth.isIn(Blocks.BARREL)) {
			directionToUse = "south";
		} else if (blockstateEast.isIn(Blocks.BARREL)) {
			directionToUse = "east";
		} else if (blockstateWest.isIn(Blocks.BARREL)) {
			directionToUse = "west";
		}
		...
		if (worldIn.isRemote) {
			...
			if (blockstateNorth.isIn(Blocks.BARREL) || blockstateSouth.isIn(Blocks.BARREL) || blockstateEast.isIn(Blocks.BARREL) || blockstateWest.isIn(Blocks.BARREL)) {
	        	 
				TileEntity tileEntity = worldIn.getTileEntity(pos.north());
	        	 
				if (directionToUse == "north") {
					tileEntity = worldIn.getTileEntity(pos.north());
				} else if (directionToUse == "south") {
					tileEntity = worldIn.getTileEntity(pos.south());
				} else if (directionToUse == "east") {
					tileEntity = worldIn.getTileEntity(pos.east());
				} else if (directionToUse == "west") {
