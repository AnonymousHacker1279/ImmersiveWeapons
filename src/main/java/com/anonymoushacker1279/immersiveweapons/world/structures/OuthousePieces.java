...
/*
public class OuthousePieces {

	private static final ResourceLocation CENTER = new ResourceLocation(ImmersiveWeapons.MOD_ID, "outhouse");
...
/**
	...
	public static void start(StructureManager structureManager, BlockPos pos, Rotation rotation, List<StructurePiece> pieceList) {
		...
		pieceList.add(new OuthousePieces.Piece(structureManager, blockPos, rotation));
	}
...
/**
		...
		Piece(StructureManager structureManager, BlockPos pos, Rotation rotationIn) {
			super(Structures.OH, 0, structureManager, OuthousePieces.CENTER, OuthousePieces.CENTER.toString(), makeSettings(rotationIn, OuthousePieces.CENTER), makePosition(pos));
		}
...
/**
		...
		public Piece(ServerLevel level, CompoundTag tag) {
			super(Structures.OH, tag, level, (resourceLocation) -> makeSettings(Rotation.valueOf(tag.getString("Rot")), resourceLocation));
		}
...
/**
		...
		private static StructurePlaceSettings makeSettings(Rotation rotationIn, ResourceLocation resourceLocationIn) {
			...
			rotation = rotationIn;
			return (new StructurePlaceSettings()).setRotation(rotationIn).setMirror(Mirror.NONE).setRotationPivot(OuthousePieces.OFFSET.get(resourceLocationIn)).addProcessor(BlockIgnoreProcessor.STRUCTURE_BLOCK);
		}
...
/**
		...
		private static BlockPos makePosition(BlockPos blockPos) {
			return blockPos.offset(OuthousePieces.OFFSET.get(OuthousePieces.CENTER));
		}
