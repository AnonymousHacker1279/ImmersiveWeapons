...
public class AbandonedFactoryPieces {
	 ...
	 */
	public static void start(StructureManager templateManager, BlockPos pos, Rotation rotation, List<StructurePiece> pieceList) {
		int x = pos.getX();
		int z = pos.getZ();

		BlockPos rotationOffset = new BlockPos(0, 0, 0).rotate(rotation);
		BlockPos blockPos = rotationOffset.offset(x, pos.getY(), z);
		pieceList.add(new AbandonedFactoryPieces.Piece(templateManager, CENTER, blockPos, rotation, GeneralUtilities.getRandomNumber(1, 3)));
	}
	...
	public static class Piece extends TemplateStructurePiece {
		...
		Piece(StructureManager templateManagerIn, ResourceLocation resourceLocationIn, BlockPos pos, Rotation rotationIn, int i) {
			super(Structures.AF, 0, templateManagerIn, resourceLocationIn, resourceLocationIn.toString(), makeSettings(rotationIn, resourceLocationIn), makePosition(resourceLocationIn, pos, i));
		}

		// TODO: javadocs
		public Piece(ServerLevel level, CompoundTag nbt) {
			super(Structures.AF, nbt, level, (resourceLocation) -> makeSettings(Rotation.valueOf(nbt.getString("Rot")), resourceLocation));
		}
		...
		private static StructurePlaceSettings makeSettings(Rotation rotationIn, ResourceLocation resourceLocationIn) {
			...
			rotation = rotationIn;
			return (new StructurePlaceSettings()).setRotation(rotationIn).setMirror(Mirror.NONE).setRotationPivot(AbandonedFactoryPieces.OFFSET.get(resourceLocationIn)).addProcessor(BlockIgnoreProcessor.STRUCTURE_BLOCK);
		}

		// TODO: javadocs
		private static BlockPos makePosition(ResourceLocation resourceLocation, BlockPos blockPos, int i) {
			return blockPos.offset(AbandonedFactoryPieces.OFFSET.get(resourceLocation)).below(i);
		}
