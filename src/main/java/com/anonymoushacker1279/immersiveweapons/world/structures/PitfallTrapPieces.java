...
/**
	...
	public static void start(StructureManager structureManager, BlockPos pos, Rotation rotation, List<StructurePiece> pieceList) {
		...
		pieceList.add(new PitfallTrapPieces.Piece(structureManager, blockPos, rotation));
	}
...
/**
		...
		Piece(StructureManager structureManager, BlockPos pos, Rotation rotationIn) {
			super(Structures.PT, 0, structureManager, PitfallTrapPieces.CENTER, PitfallTrapPieces.CENTER.toString(), makeSettings(rotationIn, PitfallTrapPieces.CENTER), makePosition(pos));
		}
...
/**
		...
		public Piece(ServerLevel level, CompoundTag tag) {
			super(Structures.PT, tag, level, (resourceLocation) -> makeSettings(Rotation.valueOf(tag.getString("Rot")), resourceLocation));
		}
...
/**
		...
		private static BlockPos makePosition(BlockPos blockPos) {
			return blockPos.offset(PitfallTrapPieces.OFFSET.get(PitfallTrapPieces.CENTER));
		}
