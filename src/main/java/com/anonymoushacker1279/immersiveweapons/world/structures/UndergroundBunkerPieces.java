...
public class UndergroundBunkerPieces {

	private static final int height = 0;

	private static final ResourceLocation CENTER = new ResourceLocation(ImmersiveWeapons.MOD_ID, "underground_bunker");
	...
	public static void start(StructureManager templateManager, BlockPos pos, Rotation rotation, List<StructurePiece> pieceList, Random random) {
		...
		pieceList.add(new UndergroundBunkerPieces.Piece(templateManager, CENTER, blockPos, rotation, GeneralUtilities.getRandomNumber(1, 3)));
	}
	...
	public static class Piece extends TemplateStructurePiece {
		...
		Piece(StructureManager templateManagerIn, ResourceLocation resourceLocationIn, BlockPos pos, Rotation rotationIn, int i) {
			super(Structures.UB, 0, templateManagerIn, resourceLocationIn, resourceLocationIn.toString(), makeSettings(rotationIn, resourceLocationIn), makePosition(resourceLocationIn, pos, i));
		}

		// TODO: javadocs
		public Piece(ServerLevel level, CompoundTag nbt) {
			super(Structures.UB, nbt, level, (resourceLocation) -> makeSettings(Rotation.valueOf(nbt.getString("Rot")), resourceLocation));
		}
		...
		private static StructurePlaceSettings makeSettings(Rotation rotationIn, ResourceLocation resourceLocationIn) {
			...
			rotation = rotationIn;
			return (new StructurePlaceSettings()).setRotation(rotation).setMirror(Mirror.NONE).setRotationPivot(UndergroundBunkerPieces.OFFSET.get(resourceLocation)).addProcessor(BlockIgnoreProcessor.STRUCTURE_BLOCK);
		}

		// TODO: javadocs
		private static BlockPos makePosition(ResourceLocation resourceLocation, BlockPos blockPos, int i) {
			return blockPos.offset(UndergroundBunkerPieces.OFFSET.get(resourceLocation)).below(i);
		}
