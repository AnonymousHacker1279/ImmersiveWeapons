	         		...
	         		if (!(f < 0.1D) && !misfire) {
	         			...
	         			if (!worldIn.isRemote) {
	         				for(int iter = 0; iter < bulletsToFire; ++iter) {
		         				...
		         				abstractarrowentity.func_234612_a_(playerentity, playerentity.rotationPitch + GeneralUtilities.getRandomNumber(-5.0f, 5.0f), playerentity.rotationYaw + GeneralUtilities.getRandomNumber(-5.0f, 5.0f), 0.0F, f * 3.0F, 1.0F);
		         				if (f == 1.0F) {
