	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, LivingEntity entityLiving, int timeLeft) {
		if (entityLiving instanceof PlayerEntity) {
	         ...
	         if (itemstack.getItem() == DeferredRegistryHandler.WOOD_MUSKET_BALL.get()) {
	        	 ...
	        	 if (randomNumber <= 3) {
	        		 misfire = true;
		             worldIn.playSound((PlayerEntity)null, playerentity.getPosX(), playerentity.getPosY(), playerentity.getPosZ(), DeferredRegistryHandler.FLINTLOCK_PISTOL_MISFIRE.get(), SoundCategory.PLAYERS, 1.0F, 1.0F / (random.nextFloat() * 0.4F + 1.2F) + 0.5F);
	         ...
	         if (!itemstack.isEmpty() || flag || !misfire) {
	            ...
	            if (!(f < 0.1D) && !misfire) {
	               ...
	               worldIn.playSound((PlayerEntity)null, playerentity.getPosX(), playerentity.getPosY(), playerentity.getPosZ(), DeferredRegistryHandler.FLINTLOCK_PISTOL_FIRE.get(), SoundCategory.PLAYERS, 1.0F, 1.0F / (random.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
