...
public class SimplePistolItem extends AbstractGunItem implements Vanishable {
	...
	ItemStack findAmmo(ItemStack itemStack, LivingEntity entityLiving) {
		...
		if (!(itemStack.getItem() instanceof AbstractGunItem)) {
			...
			Predicate<ItemStack> predicate = ((AbstractGunItem) itemStack.getItem()).getAmmoPredicate();
			ItemStack itemStack2 = AbstractGunItem.getHeldAmmo(playerEntity, predicate);
			if (!itemStack2.isEmpty()) {
				return itemStack2;
			} else {
				predicate = ((AbstractGunItem) itemStack.getItem()).getInventoryAmmoPredicate();
